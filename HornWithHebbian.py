...
class HeterogeneousHORNWithHebbian(nn.Module):
    def __init__(self, num_nodes, base_omega, base_gamma, base_alpha, sequence_length, output_classes=10, hetero_std=0.01):
        super(HeterogeneousHORNWithHebbian, self).__init__()
        self.num_nodes = num_nodes
        self.sequence_length = sequence_length
        self.h = 0.1  # Reduced integration step
        ...
        self.W_hh = nn.Linear(num_nodes, num_nodes, bias=False)  # Hebbian-trained weights
    ...
    def hebbian_update(self, x, y, eta=1e-4):  
        with torch.no_grad():
            delta_W_hh = eta * torch.outer(y, x)
            # Option 1: Normalize delta_W_hh
            # delta_W_hh = delta_W_hh / (torch.norm(delta_W_hh) + 1e-8)
            # Option 2: Clip weights after update
            updated_weight = self.W_hh.weight + delta_W_hh
            updated_weight = torch.clamp(updated_weight, -1.0, 1.0)  # Example clipping
            self.W_hh.weight = nn.Parameter(updated_weight)
    ...
    def forward(self, input_sequence):
        ...
        for t in range(self.sequence_length):
            ...
            x = x.clone() + self.h * y
            y = y.clone() + self.h * accel

            self.hebbian_update(x.mean(dim=0), y.mean(dim=0))  # Updated Hebbian update
...
def train_with_hebbian(horn_network, train_loader, test_loader, epochs=100, lr=0.001):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.title('Heterogeneous HORN with Hebbian Plasticity')
    plt.legend()
    plt.grid(True)
    plt.show()
...
if __name__ == "__main__":
    ...
    horn_network = HeterogeneousHORNWithHebbian(num_nodes, base_omega, base_gamma, base_alpha, sequence_length)
    train_accuracies, test_accuracies, losses = train_with_hebbian(horn_network, train_loader, test_loader, epochs=100, lr=0.001)
